...
class Car16010 extends Car {
	private Location setter;
	static int count_car = 160100;
	private int status=1;
	private Trip tri;
	private double x1,y1;

	public Car16010(int speed) {
	...
	public int distSqrd(Location loc) {
		return (int) Math.sqrt(Math.pow(setter.getX() - loc.getX(), 2) + Math.pow(setter.getY() - loc.getY(), 2));
	}
	@Override
